...
description: 🎨 Academic Classic presentation template — red-yellow palette, fixed 1920x1080 canvas, dynamic multi-logo, amber-gold progress bar. Single unified template with 9 skins.
---

# Academic Classic — Full Style Specification

> **Reference spec for Academic Classic.** CSS/JS source of truth is the template file (`assets/templates/Empty_template.html`). This doc covers: class index, HTML patterns, layout systems, generation rules, and the Element Annotation Convention (EAC).
...
|---|----------|----------------|---------|
| Q0 | Logo selection: scan `assets/logos/`, ask which logos to use | Logo row | Default: CMS |
...
**Naming convention:** `{Name}_logo.{ext}` (e.g. `CERN_logo.png`, `CMS_logo.png`, `ATLAS_logo.svg`)
...
```html
<img src="/abs/path/to/assets/logos/CERN_logo.png" class="logo-img" alt="CERN logo">
...
- Square logos (ratio 0.9–1.1) → `border-radius: 50%` (circular, e.g., university seals, CEPC)
...
|-----------|---------|--------|
| `--logos CMS_logo.png` | Default | CMS |
| `--logos CERN_logo.png CEPC_logo.png` | CEPC mode | seal (circle) + CEPC (circle) |
| `--logos CERN_logo.png CMS_logo.png ATLAS_logo.svg` | Multi-experiment | 3 logos, auto-sized |
| (omit `--logos`) | Default | CMS only |
...
> 10 color skins are defined in `assets/skins/*.css`, see [SKINS.md](SKINS.md) for details.
> Injected via `init-slides.py --skin <name>`. Default `classic` = Red-Yellow-White (no overrides).
>
